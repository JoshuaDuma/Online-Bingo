...
function constantGrid(array) {
  ...
  return newObj;
} // Craete the same grid for every player.
...
fun.query('SELECT * FROM fun.fun', function (err, result, fields) {
  if (err) throw err;
  if (result.length > 0) {
...
io.on('connection', function (socket) {
  ...
  socket.on('callAnswer', function (response) {
    if (admin) {
  ...
  socket.on('adminCtl', function (response) {
